window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = `
      ...
      .dsh-provider-usage-badge[data-status="loading"] {
        color: var(--dsw-alias-label-caption);
      }
      .dsh-provider-usage-badge[data-level="warn"] {
        color: var(--dsw-alias-state-warn-label);
        border-color: var(--dsw-alias-state-warn-label);
      }
      .dsh-provider-usage-badge[data-level="danger"] {
        color: var(--dsw-alias-state-error-primary);
        border-color: var(--dsw-alias-state-error-primary);
      }
      .dsh-provider-usage-percent { font-variant-numeric: tabular-nums; font-weight: 600; }
      .dsh-provider-usage-reset { color: var(--dsw-alias-label-caption); }
    `;
    const styleId = "@local/dsh-provider-usage/provider-usage.css";
    if (typeof document !== "undefined" && document.querySelector(`style[data-plugin-css="${styleId}"]`) === null) {
      const tag = document.createElement("style");
      tag.dataset.plugin = "@local/dsh-provider-usage";
    ...
    async function fetchUsage(provider, signal) {
      const response = await fetch(`/provider-usage?provider=${encodeURIComponent(provider)}`, {
    ...
    function ProviderUsageBadge({ provider }) {
      ...
      if (!window) {
        return jsx.jsx("span", {
          className: "dsh-provider-usage-badge",
          "data-status": "loading",
      ...
      return jsx.jsxs("span", {
        ...
        "aria-label": title,
        children: [
          jsx.jsx("span", { className: "dsh-provider-usage-percent", children: `${remaining}%` }),
          jsx.jsx("span", { className: "dsh-provider-usage-reset", children: countdown }),
        ],
      });
    }
    ...
    function apply(ctx) {
      ctx.inject(["slots", "modelDirectories"], (scope) => {
        ...
        scope.slots.inject("conversation.input.right", () => scope.slots.register({
          ...
          id: "@local/dsh-provider-usage:badge",
          order: 10000,
          inject: (sessionId) => ({
            directory: directories.directoryFor(sessionId).store,
          }),
        }, function ProviderUsageSeat({ directory }) {
          ...
          return jsx.jsx(ProviderUsageBadge, { provider });
        }));
      });
    }
